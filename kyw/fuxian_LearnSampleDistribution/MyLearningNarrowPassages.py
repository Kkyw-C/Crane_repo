...
def test(c_test, c_gapsInitGoal, occGridSamples):
    dim = 6
    gridSize = 11
...
def train():
    ...
    for it in range(it, it + 500001):
        ...
        if it % 10000 == 0:
            saver.save(sess, 'tf_models/my-model', global_step=10000)
            test(c_test, c_gapsInitGoal, occGridSamples)



def run_test():
    saver = tf.train.Saver()
    saver.restore(sess, "tf_models/my-model-10000")
    for i in range(5):
        test(c_test, c_gapsInitGoal, occGridSamples)
